# app.py — robust, crash-proofed Streamlit app (global guard, circuit breaker, jump-safe)
import io, json, time, hashlib, ssl, re, random, traceback
...
HTTP_TIMEOUT = 12  # for thumbnail fetch
BTN_MIN_INTERVAL = 0.85
BTN_GLOBAL_INTERVAL = 0.55
RERUN_STORM_INTERVAL = 0.25  # ignore rerun requests faster than this
GDRIVE_MAX_RETRIES = 6
CB_FAIL_WINDOW_SEC = 30
CB_TRIP_THRESHOLD = 5
CB_COOLDOWN_SEC = 20
ROWS_PER_PROMPT_DEFAULT = 5
# ============================================
...
def _jitter(x: float, j: float = 0.2) -> float: return x * (1.0 + random.uniform(-j, j))

def _init_state():
    ss = st.session_state
    ss.setdefault("_btn_last_click", {})
    ss.setdefault("_global_cooldown_until", 0.0)
    ss.setdefault("_save_lock", False)
    ss.setdefault("_io_lock", False)
    ss.setdefault("_last_rerun_ts", 0.0)
    ss.setdefault("_drive_fail_count", 0)
    ss.setdefault("_cb_trip_until", 0.0)
    ss.setdefault("user", None)
    ss.setdefault("allowed", [])
    ...
    ss.setdefault("_jump_anchor_idx", None)
    ss.setdefault("_last_meta_len", 0)

def rerun_guard() -> bool:
    # Returns True if we should stop to avoid rerun storm
    now = _now()
    last = st.session_state._last_rerun_ts
    if now - last < RERUN_STORM_INTERVAL:
        return True
    st.session_state._last_rerun_ts = now
    return False

def safe_button(label: str, key: str, min_interval: float = BTN_MIN_INTERVAL,
                global_interval: float = BTN_GLOBAL_INTERVAL, **kwargs) -> bool:
...
# ----------------- CIRCUIT BREAKER for Drive -----------------
def _cb_open() -> bool:
    return _now() < st.session_state._cb_trip_until

def _cb_record_failure():
    # decay old failures
    st.session_state._drive_fail_count += 1
    if st.session_state._drive_fail_count >= CB_TRIP_THRESHOLD:
        st.session_state._cb_trip_until = _now() + CB_COOLDOWN_SEC
        st.session_state._drive_fail_count = 0

def _cb_record_success():
    st.session_state._drive_fail_count = max(0, st.session_state._drive_fail_count - 1)
...
def _ensure_drive():
    ...
    sec = st.secrets.get("gcp", {})
    if not sec:
        raise RuntimeError("Missing gcp secrets")
    sa_raw = sec.get("service_account")
    if not sa_raw:
        raise RuntimeError("Missing gcp.service_account")
...
def get_drive():
    global drive
    if _cb_open():
        raise RuntimeError("Drive temporarily unavailable (cooldown).")
    if drive is None:
        drive = _ensure_drive()
    return drive
...
def _download_bytes_with_retry(file_id: str, attempts: int = GDRIVE_MAX_RETRIES) -> bytes:
    last_err = None
...
def read_text_from_drive(file_id: str) -> str:
    try:
        data = _download_bytes_with_retry(file_id)
        ...
        if cached is not None:
            st.info("Drive hiccup — using cached contents.")
            return cached
        raise

def write_text_to_drive(file_id: str, text: str, attempts: int = 4):
    ...
    try:
        ...
        for i in range(attempts):
            try:
                ...
                drv.files().update(fileId=file_id, media_body=media,
...
def append_lines_to_drive_text(file_id: str, new_lines: List[str], retries: int = 3):
    for attempt in range(retries):
        try:
            prev = read_text_from_drive(file_id)
            updated = prev + "".join(new_lines)
            write_text_to_drive(file_id, updated)
            return
        except Exception:
...
def find_file_id_in_folder(folder_id: Optional[str], filename: str) -> Optional[str]:
...
def delete_file_by_id(file_id: Optional[str]):
...
def create_shortcut_to_file(src_file_id: Optional[str], new_name: str, dest_folder_id: Optional[str]) -> Optional[str]:
    if not (src_file_id and dest_folder_id):
        return None
    try:
        drv = get_drive()
        meta = {"name": new_name, "mimeType": "application/vnd.google-apps.shortcut",
                "parents": [dest_folder_id], "shortcutDetails": {"targetId": src_file_id}}
        res = drv.files().create(body=meta, fields="id,name", supportsAllDrives=True).execute()
        _cb_record_success()
        return res.get("id")
    except Exception:
        _cb_record_failure()
...
def progress_file_id_for(cat: str, who: str) -> Optional[str]:
    try:
        sec = st.secrets.get("gcp", {})
        parent = sec.get("progress_parent_id") or sec.get(f"{cat}_hypo_filtered_log_id")
        if not parent: return None
        fname = f"progress_{canonical_user(who)}_{cat}.txt"
        drv = get_drive()
        q = f"'{parent}' in parents and name = '{fname}' and trashed = false"
        resp = drv.files().list(q=q, fields="files(id,name)", pageSize=1,
        ...
        res = drv.files().create(body=meta, media_body=media, fields="id",
                                 supportsAllDrives=True).execute()
        _cb_record_success()
        return res["id"]
    except Exception:
...
def persist_pointer(cat: str, who: str, idx: int):
    ...
    try:
        write_text_to_drive(pfid, str(int(max(0, idx))))
    except Exception:
        pass
...
def preview_bytes(file_id: str, max_side: int = 680) -> bytes:
    ...
    src = tb if tb is not None else _download_bytes_with_retry(file_id)
...
def original_bytes(file_id: str) -> bytes:
    return _download_bytes_with_retry(file_id)
...
CAT = {
    ...
    "objects": {
        ...
        "log_adv":  _safe_secret("objects_adv_filtered_log_id"),
...
def load_meta(jsonl_id: Optional[str]) -> List[Dict[str, Any]]:
    if not jsonl_id:
        return []
    try:
        drv = get_drive()
        drv.files().get(fileId=jsonl_id, fields="id", supportsAllDrives=True).execute()
    except Exception:
        return []
    try:
        raw = read_text_from_drive(jsonl_id)
    except Exception:
        return []
...
@st.cache_data(show_spinner=False)
def load_latest_map_for_annotator(log_file_id: Optional[str], who: str) -> Dict[str, Dict]:
    if not log_file_id:
        return {}
    try:
        rows = latest_rows(read_text_from_drive(log_file_id))
    except Exception:
        rows = []
...
def build_completion_sets(cat_cfg: dict, who: str) -> Tuple[set, Dict[str, Dict], Dict[str, Dict]]:
    log_h_map = load_latest_map_for_annotator(cat_cfg.get("log_hypo"), who)
    log_a_map = load_latest_map_for_annotator(cat_cfg.get("log_adv"),  who)
    completed = set()
...
@st.cache_data(show_spinner=False)
def count_records_for_annotator(log_file_id: Optional[str], who: str) -> int:
    if not log_file_id:
        return 0
    try:
        text = read_text_from_drive(log_file_id)
    except Exception:
        return 0
...
def first_undecided_index_from_counts(meta_len: int, hypo_log_id: Optional[str], adv_log_id: Optional[str], who: str) -> int:
...
def rows_per_prompt() -> int:
    try:
        return int(st.secrets.get("app", {}).get("rows_per_prompt", ROWS_PER_PROMPT_DEFAULT))
    except Exception:
        return ROWS_PER_PROMPT_DEFAULT
...
def prompt_to_first_row_idx(n: int, total_len: int) -> int:
    idx0 = (n - 1) * rows_per_prompt()  # FIRST triplet of the prompt (0-based)
...
def main():
    ...
    # --- Right panel (status & jump) ---
    with right:
        allowed = st.session_state.get("allowed", [])
        if not allowed:
            st.error("No categories assigned to this user."); st.stop()

        # guard for invalid cat in session
        if st.session_state.cat not in allowed:
            st.session_state.cat = allowed[0]
        ...
        cfg  = CAT.get(st.session_state.cat, {})
        meta = load_meta(cfg.get("jsonl_id"))
        st.session_state._last_meta_len = len(meta)

        done_h = count_records_for_annotator(cfg.get("log_hypo"), who)
        done_a = count_records_for_annotator(cfg.get("log_adv"),  who)
        ...
        if safe_button("Go", key="jump_go_btn"):
            if not meta:
                ...
                if n is None:
                    ...
                    st.session_state._jump_anchor_idx = base_idx
                    persist_pointer(st.session_state.cat, st.session_state.user, base_idx)
                    if not rerun_guard():
                        st.rerun()

    # Auto-position on first load
    if st.session_state.idx_initialized_for != st.session_state.cat:
        cfg_init  = CAT.get(st.session_state.cat, {})
        meta_init = load_meta(cfg_init.get("jsonl_id"))
        idx = first_undecided_index_from_counts(len(meta_init), cfg_init.get("log_hypo"), cfg_init.get("log_adv"), st.session_state.user)
    ...
    with left:
        ...
        src_h_id = find_file_id_in_folder(cfg.get("src_hypo"), hypo_name)
        src_a_id = find_file_id_in_folder(cfg.get("src_adv"),  adv_name)
        ...
        with imgL:
            ...
            with b1:
                acc_key = f"acc_h_{pk}"
                if safe_button("✅ Accept (hypo)", key=acc_key):
                    st.session_state.dec[pk]["hypo"] = "accepted"
            with b2:
                rej_key = f"rej_h_{pk}"
                if safe_button("❌ Reject (hypo)", key=rej_key):
        ...
        with imgR:
            ...
            with b3:
                ...
                if safe_button("✅ Accept (adv)", key=acca_key):
                    st.session_state.dec[pk]["adv"] = "accepted"
            with b4:
                reja_key = f"rej_a_{pk}"
                if safe_button("❌ Reject (adv)", key=reja_key):
            ...
            try:
                ...
                try:
                    if (saved_h == "accepted") and (new_h_status != "accepted"):
                        delete_file_by_id(prev_h_copied or find_file_id_in_folder(cfg.get("dst_hypo"), hypo_name))
                        new_h_copied = None
                    if new_h_status == "accepted":
                        delete_file_by_id(prev_h_copied or find_file_id_in_folder(cfg.get("dst_hypo"), hypo_name))
                        if src_h_id:
                            new_h_copied = create_shortcut_to_file(src_h_id, hypo_name, cfg.get("dst_hypo"))

                    if (saved_a == "accepted") and (new_a_status != "accepted"):
                        delete_file_by_id(prev_a_copied or find_file_id_in_folder(cfg.get("dst_adv"), adv_name))
                        new_a_copied = None
                    if new_a_status == "accepted":
                        delete_file_by_id(prev_a_copied or find_file_id_in_folder(cfg.get("dst_adv"), adv_name))
                        if src_a_id:
                            new_a_copied = create_shortcut_to_file(src_a_id, adv_name, cfg.get("dst_adv"))
                except Exception as e:
                ...
                try:
                    append_lines_to_drive_text(cfg.get("log_hypo"), [json.dumps(rec_h, ensure_ascii=False) + "\n"])
                    append_lines_to_drive_text(cfg.get("log_adv"),  [json.dumps(rec_a, ensure_ascii=False) + "\n"])
                except Exception as e:
                ...
                try:
                    meta_local = load_meta(cfg.get("jsonl_id"))
        ...
        with navL:
            prev_key = f"prev_{pk}"
            if safe_button("⏮ Prev", key=prev_key):
                st.session_state.idx = clamp_idx(i - 1, len(meta))
                persist_pointer(st.session_state.cat, st.session_state.user, st.session_state.idx)
                if not rerun_guard():
                    st.rerun()
        ...
        with navC:
            ...
            if safe_button("💾 Save", key="save_btn", type="primary", disabled=disabled_save, use_container_width=True):
                with st.spinner("Saving..."):
                    save_now()

        with navR:
            next_key = f"next_{pk}"
            if safe_button("Next ⏭", key=next_key):
                st.session_state.idx = clamp_idx(i + 1, len(meta))
                persist_pointer(st.session_state.cat, st.session_state.user, st.session_state.idx)
                if not rerun_guard():
                    st.rerun()
...
try:
    ...
    st.error("The app hit a recoverable error. Nothing was lost. You can continue.")
